...
def main():
    ...
    with open('input.txt', 'rb') as file:
        ...
        while True:
            ...
            try:
                ...
                print(f'Timeout, reenviando a partir de seq={current_seq}')
                for seq, packet_data in sliding_window:
                    sock.sendto(packet_data, (SERVER_IP, PORT))
                    print(f'Reenviado quadro com seq={seq}')
    ...
    while not acked:
        ...
        sock.sendto(packet, (SERVER_IP, PORT))
        print(f'Enviado quadro de encerramento com seq={next_seq}')
        try:
            ack, _ = sock.recvfrom(1)
            if int.from_bytes(ack, 'big') == next_seq:
                acked = True
                print(f'ACK {next_seq} recebido para encerramento')
        except socket.timeout:
            print(f'Timeout, reenviando encerramento')
