...
Avaliar a eficiência dos protocolos **Go-Back-N** ou **Selective Repeat** (comparado com Stop-and-Wait), com execução de testes controlados no Mininet em cenários de rede com diferentes **larguras de banda (Mbps)**, **atrasos (ms)** e **taxas de perda (%)**.
...
- Arquivos de exemplo para transmissão (`input.txt`, `input_02.txt`)
- Use apenas apenas o `input_02.txt` para coletar as métricas
...
```bash
sudo mn --custom topo_stopandwait.py --topo stopandwait --link tc 
```
...
- Implementação completa do protocolo Go-Back-N (GBN) ou Selective Repeat (SR)
